...
func heapSort(numbers: inout [Int]) {
  ...
  func siftDown(fromIdx index: Int, upTo size: Int) {
  ...
  for i in stride(from: numbers.count / 2 + 1, through: 0, by: -1) {
    siftDown(fromIdx: i, upTo: numbers.count)
  }
  
  for i in stride(from: numbers.count - 1, through: 0, by: -1) {
    numbers.swapAt(0, i)
    siftDown(fromIdx: 0, upTo: i)
  }
}


func dutch_flag_sort(balls: inout [Character]) {
  var blueLeadingEdge = 0
  var greenLeadingEdge = 0
  
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, blueLeadingEdge)
        balls.swapAt(blueLeadingEdge, greenLeadingEdge)
        blueLeadingEdge += 1
        greenLeadingEdge += 1
      case "G":
        balls.swapAt(i, blueLeadingEdge)
