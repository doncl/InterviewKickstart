...
func quickSortLomuto(numbers: inout [Int]) {
  func helper(start: Int, end: Int) {
    ...
    let randomMidpointIdx = Int.random(in: start...end)
    numbers.swapAt(start, randomMidpointIdx)
...
func dutch_flag_sort(balls: inout [Character]) {
  var leadingGreenEdge = 0
  var leadingBlueEdge = 0
  
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, leadingBlueEdge)
        balls.swapAt(leadingGreenEdge, leadingBlueEdge)
        leadingBlueEdge += 1
        leadingGreenEdge += 1
