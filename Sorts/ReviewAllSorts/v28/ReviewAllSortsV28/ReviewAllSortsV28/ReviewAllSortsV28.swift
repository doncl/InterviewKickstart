//
//  ReviewAllSortsV28.swift
//  ReviewAllSortsV28
//
//  Created by Don Clore on 3/4/23.
//
...
func dutch_flag_sort(balls: inout [Character]) {
  var blueThresholdStart = 0
  var greenThresholdStart = 0
  
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, blueThresholdStart)
        balls.swapAt(blueThresholdStart, greenThresholdStart)
        blueThresholdStart += 1
        greenThresholdStart += 1
...
func segregate_evens_and_odds(numbers: inout [Int]) {
  ...
  for i in 0..<numbers.count {
    let num = numbers[i]
    if num % 2 == 0 {
      numbers.swapAt(i, leftThreshold)
      leftThreshold += 1
    }
  }
}
