...
func heapSort(numbers: inout [Int]) {
  func getLeftIndex(forParentIndex index: Int) -> Int {
    index * 2 + 1
  }
  func getRightIndex(forParentIndex index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChildIdx = getLeftIndex(forParentIndex: parentIndex)
      let rightChildIdx = getRightIndex(forParentIndex: parentIndex)
      
      var candidateIndex = parentIndex
      
      if leftChildIdx < size && numbers[leftChildIdx] > numbers[candidateIndex] {
        candidateIndex = leftChildIdx
      }
      
      if rightChildIdx < size && numbers[rightChildIdx] > numbers[candidateIndex] {
