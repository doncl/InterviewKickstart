...
func quickSortLomuto(numbers: inout [Int]) {
  func helper(start: Int, end: Int) {
    ...
    let randomMidIdx = Int.random(in: start...end)
    numbers.swapAt(start, randomMidIdx)
...
func dutch_flag_sort(balls: inout [Character]) {
  ...
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, blueLeftEdge)
        balls.swapAt(blueLeftEdge, greenleftEdge)
        blueLeftEdge += 1
        greenleftEdge += 1
      case "G":
        balls.swapAt(i, blueLeftEdge)
