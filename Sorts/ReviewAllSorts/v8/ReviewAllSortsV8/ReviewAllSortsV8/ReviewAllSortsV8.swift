...
func heapSort(numbers: inout [Int]) {
  func getLeftChildIndex(forParentIndex index: Int) -> Int {
    index * 2 + 1
  }
  
  func getRightChildIndex(forParentIndex index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChildIndex(forParentIndex: parentIndex)
      let rightChild = getRightChildIndex(forParentIndex: parentIndex)
