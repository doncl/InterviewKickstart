...
func quickSortLomuto(numbers: inout [Int]) {
  func helper(start: Int, end: Int) {
    ...
    numbers.swapAt(randomPivotIndex, start)
...
func heapSort(numbers: inout [Int]) {
  func getLeftChild(forParentIndex index: Int) -> Int {
    index * 2 + 1
  }
  func getRightChild(forParentIndex index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChild(forParentIndex: parentIndex)
      let rightChild = getRightChild(forParentIndex: parentIndex)
