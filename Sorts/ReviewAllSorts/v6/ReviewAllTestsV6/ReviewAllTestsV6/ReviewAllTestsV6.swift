...
func quickSortLomuto(numbers: inout [Int]) {
  func helper(start: Int, end: Int) {
    ...
    numbers.swapAt(start, randomIdx)
...
func heapSort(numbers: inout [Int]) {
  func getLeftChild(forParent index: Int) -> Int {
    index * 2 + 1
  }
  
  func getRightChild(forParent index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChild(forParent: parentIndex)
      let rightChild = getRightChild(forParent: parentIndex)
