...
func heapSort(numbers: inout [Int]) {
  func getLeftChildIdx(forParendIdx index: Int) -> Int {
    index * 2 + 1
  }
  
  func getRightChildIdx(forParendIdx index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(at index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChildIdx(forParendIdx: parentIndex)
      let rightChild = getRightChildIdx(forParendIdx: parentIndex)
