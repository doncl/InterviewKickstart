//
//  ReviewAllSortsV23.swift
//  ReviewAllSortsV23
//
//  Created by Don Clore on 2/23/23.
//
...
func mergeSort(numbers: inout [Int]) {
  func helper(start: Int, end: Int) {
    ...
    while leftIdx <= midpoint && rightIdx <= end {
      let leftNumber = numbers[leftIdx]
      let rightNumber = numbers[rightIdx]
      
      if leftNumber <= rightNumber {
        aux.append(leftNumber)
        leftIdx += 1
      } else {
        aux.append(rightNumber)
        rightIdx += 1
      }
    }
...
func dutch_flag_sort(balls: inout [Character]) {
  var greenRightEdge = 0
  var blueRightEdge = 0
  
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, blueRightEdge)
        balls.swapAt(blueRightEdge, greenRightEdge)
        blueRightEdge += 1
        greenRightEdge += 1
