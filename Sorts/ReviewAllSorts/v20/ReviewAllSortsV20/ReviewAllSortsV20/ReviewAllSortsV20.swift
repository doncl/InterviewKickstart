...
func heapSort(numbers: inout [Int]) {
  func getLeftChildIdx(fromParentIdx index: Int) -> Int {
    index * 2 + 1
  }
  
  func getRightChildIdx(fromParentIdx index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChildIdx(fromParentIdx: parentIndex)
      let rightChild = getRightChildIdx(fromParentIdx: parentIndex)
