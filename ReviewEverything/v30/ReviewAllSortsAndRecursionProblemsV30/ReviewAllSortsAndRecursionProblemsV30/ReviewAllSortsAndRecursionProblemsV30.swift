...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  var build: [Int] = []
  let remaining: Int = bank.reduce(0, +)
...
struct Q {
  ...
  mutating func enQueue(_ n: BinaryTreeNode) {
    right.append(n)
  }
...
func delete_from_bst(root: BinaryTreeNode?, values_to_be_deleted: [Int]) -> BinaryTreeNode? {
  ...
  func search(root: BinaryTreeNode?, value: Int) -> (nodeToDelete: BinaryTreeNode, parent: BinaryTreeNode?)? {
    guard let root = root else {
      return nil
    }
    var parent: BinaryTreeNode? = nil
    var curr: BinaryTreeNode? = root
    
    while curr != nil {
      if curr!.value == value {
        return (nodeToDelete: curr!, parent: parent)
  ...
  func deleteFromBst(root: BinaryTreeNode?, value: Int) -> BinaryTreeNode? {
    ...
    guard let nodeToDeleteTuple = search(root: root, value: value) else {
      return root
    }
    
    let nodeToDelete = nodeToDeleteTuple.nodeToDelete
    var parent = nodeToDeleteTuple.parent
    ...
    // CASE 3
    if let successor = getSuccessor(node: nodeToDelete) {
  ...
  for valueToDelete in values_to_be_deleted {
    ret = deleteFromBst(root: ret, value: valueToDelete)
  }
