//
//  ReviewAllSortsAndRecursionProblemsV7.swift
//  ReviewAllSortsAndRecursionProblemsV7
//
...
func decimalStringsNoRepetition(n: Int) -> [String] {
  ...
  func helper(build: inout String) {
    ...
    for i in 0..<digits.count {
      var stringIndex = digits.index(digits.startIndex, offsetBy: i)
...
func generateIntSubsetsAllowDuplicates(arr: [Int]) -> [[Int]] {
  ...
  func helper(build: inout [Int], index: Int) {
    ...
    while loopindex < arr.count && arr[loopindex] == candidate {
      loopindex += 1
      count += 1
    }
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], bank: [Int], index: Int, remaining: Int, target: Int) {
    guard target != 0 else {
      ret.append(build)
      return
    }
    ...
    while endOfDupsIndex < bank.count && bank[endOfDupsIndex] == candidate {
      endOfDupsIndex += 1
      dupsCount += 1
    }
    
    var amountToRemove = dupsCount * candidate
    var exclusionRemaining = remaining - amountToRemove
    helper(build: &build, bank: bank, index: endOfDupsIndex, remaining: exclusionRemaining, target: target)
        
    for i in 1...dupsCount {
      build.append(candidate)
      let localSum = i * candidate
      let localRemaining = remaining - localSum
      let localTarget = target - localSum
      helper(build: &build, bank: bank, index: endOfDupsIndex, remaining: localRemaining, target: localTarget)
    }
  ...
  helper(build: &build, bank: bank, index: 0, remaining: sum, target: target)
  return ret
}
...
func generate_palindromic_decompositions(s: String) -> [String] {
  ...
  func helper(build: inout String, index: Int) {
    ...
    for i in index..<bank.count {
      let characterCandidate = bank[i]
      palindrome.append(characterCandidate)
