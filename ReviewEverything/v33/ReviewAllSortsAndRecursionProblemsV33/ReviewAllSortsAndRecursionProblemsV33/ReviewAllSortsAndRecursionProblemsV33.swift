//
//  ReviewAllSortsAndRecursionProblemsV33.swift
//  ReviewAllSortsAndRecursionProblemsV33
//
...
func decimalStringsNoRepetition(n: Int) -> [String] {
  ...
  func helper(build: inout String, index: Int) {
    ...
    for i in 0..<digits.count {
      ...
      let digit = digits[stringIndex]
      build.append(digit)
      digits.remove(at: stringIndex)
      helper(build: &build, index: index + 1)
      digits.insert(digit, at: stringIndex)
      _ = build.popLast()
    }
  }
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], index: Int, remainder: Int, target: Int) {
    guard target != 0 else {
      ret.append(build)
      return
    }
    ...
    // Exclusion remaining
    let exclusionRemaining = remainder - (count * candidate)
    helper(build: &build, index: loopindex, remainder: exclusionRemaining, target: target)
    
    for i in 1...count {
      build.append(candidate)
      let localSum = candidate * i
      let localRemaining = remainder - localSum
      let localTarget = target - localSum
      helper(build: &build, index: loopindex, remainder: localRemaining, target: localTarget)
    }
  ...
  var build: [Int] = []
  let remainder = bank.reduce(0, +)
  helper(build: &build, index: 0, remainder: remainder, target: target)
...
func delete_from_bst(root: BinaryTreeNode?, values_to_be_deleted: [Int]) -> BinaryTreeNode? {
  ...
  func deleteFromBST(root: BinaryTreeNode?, value: Int) -> BinaryTreeNode? {
    ...
    if let child {
      ...
      if child.value < nodeToDelete.value {
        parent.left = child
      } else {
        parent.right = child
      }
