//
//  ReviewAllSortsAndRecursionProblemsV16.swift
//  ReviewAllSortsAndRecursionProblemsV16
//
//  Created by Don Clore on 3/21/23.
//
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], index: Int, remainder: Int, target: Int) {
    ...
    for i in 1...count {
      ...
      let localRemainder = remainder - localSum
      let localTarget = target - localSum
      helper(build: &build, index: loopindex, remainder: localRemainder, target: localTarget)
    }
