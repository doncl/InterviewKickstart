...
func get_words_from_phone_number(phone_number: String) -> [String] {
  ...
  func helper(build: inout String, index: Int) {
    ...
    let candidate = phone_number[stringindex]
    if let letterArray = map[candidate] {
...
func find_all_arrangements(n: Int) -> [[String]] {
  ...
  func isConflicted(lastRow: Int, lastColumn: Int, previousRow: Int, previousColumn: Int) -> Bool {
    guard lastColumn != previousColumn else {
      return true  // it is conflicted
    }
    // check for diagonal conflict
    let columnDiff = abs(lastColumn - previousColumn)
    let rowDiff = abs(lastRow - previousRow)
  ...
  func helper(build: inout [Int], index: Int) {
    ...
    if index > 1 {
      let lastRow = index - 1
      let lastColumn = build[index - 1]
      
      for i in 0..<index - 1 {
        let earlierRow = i
        let earlierColumn = build[i]
        
        if isConflicted(lastRow: lastRow, lastColumn: lastColumn, previousRow: earlierRow, previousColumn: earlierColumn) {
          return
        }
      }
    }
