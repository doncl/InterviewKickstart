//
//  ReviewAllSortsAndRecursionProblemsV20.swift
//  ReviewAllSortsAndRecursionProblemsV20
//
...
func find_all_arrangements(n: Int) -> [[String]] {
  ...
  func helper(build: inout [Int], index: Int) {
    if index > 1 {
      ...
      for i in 0..<lastRow {
        let previousRow = i
        let previousColumn = build[i]
        
        if isConflicted(lastRow: lastRow, lastColumn: lastColumn, earlierRow: previousRow, earlierColumn: previousColumn) {
          return
        }
      }
    }
