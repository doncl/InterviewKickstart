//
//  ReviewAllSortsAndRecursionProblemsV6.swift
//  ReviewAllSortsAndRecursionProblemsV6
//
//  Created by Don Clore on 3/18/23.
//
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], bank: [Int], index: Int, remaining: Int, target: Int) {
    ...
    let totalDupsAmount = dupsCount * candidate
    let remainingAfterDups = remaining - totalDupsAmount
    
    // Exclusion case
    helper(build: &build, bank: bank, index: endOfDupsIndex, remaining: remainingAfterDups, target: target)
  ...
  var build: [Int] = []
  let currentSum = arr.reduce(0, +)
  let bank = arr.sorted()
  
  helper(build: &build, bank: bank, index: 0, remaining: currentSum, target: target)
