//
//  ReviewAllSortsAndRecursionProblemsV15.swift
//  ReviewAllSortsAndRecursionProblemsV15
//
...
func heapSort(numbers: inout [Int]) {
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      ...
      var candidateIndx = parentIndex
      
      if leftChild < size && numbers[leftChild] > numbers[candidateIndx] {
        candidateIndx = leftChild
      }
      
      if rightChild < size && numbers[rightChild] > numbers[candidateIndx] {
        candidateIndx = rightChild
      }
      
      guard parentIndex != candidateIndx else {
        return
      }
      
      numbers.swapAt(candidateIndx, parentIndex)
      parentIndex = candidateIndx
    }
  }
