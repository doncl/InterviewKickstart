//
//  ReviewAllSortsAndRecursionProblemsV25.swift
//  ReviewAllSortsAndRecursionProblemsV25
//
//  Created by Don Clore on 3/29/23.
//
...
func heapSort(numbers: inout [Int]) {
  func getLeftChildIndex(forParentIdx index: Int) -> Int {
    index * 2 + 1
  }
  
  func getRightChildIndex(forParentIdx index: Int) -> Int {
    index * 2 + 2
  }
  ...
  func siftDown(from index: Int, upTo size: Int) {
    ...
    while true {
      let leftChild = getLeftChildIndex(forParentIdx: parentIndex)
      let rightChild = getRightChildIndex(forParentIdx: parentIndex)
...
func find_all_arrangements(n: Int) -> [[String]] {
  ...
  func isConflict(lastRow: Int, lastColumn: Int, earlierRow: Int, earlierColumn: Int) -> Bool {
