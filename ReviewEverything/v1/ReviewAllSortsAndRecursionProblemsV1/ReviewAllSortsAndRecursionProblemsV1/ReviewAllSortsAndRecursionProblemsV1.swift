//
//  ReviewAllSortsAndRecursionProblemsV1.swift
//  ReviewAllSortsAndRecursionProblemsV1
//
//  Created by Don Clore on 3/13/23.
//
...
func dutch_flag_sort(balls: inout [Character]) {
  var blueLeftThreshold = 0
  var greenLeftThreshold = 0
  
  for i in 0..<balls.count {
    switch balls[i] {
      case "R":
        balls.swapAt(i, blueLeftThreshold)
        balls.swapAt(blueLeftThreshold, greenLeftThreshold)
        blueLeftThreshold += 1
        greenLeftThreshold += 1
...
func generateIntSubsetsAllowDuplicates(arr: [Int]) -> [[Int]] {
  ...
  func helper(build: inout [Int], bank: [Int], index: Int) {
    ...
    while floatingIndex < bank.count && bank[floatingIndex] == candidate {
      floatingIndex += 1
      count += 1
    }
  ...
  var build: [Int] = []
  helper(build: &build, bank: arr.sorted(), index: 0)
