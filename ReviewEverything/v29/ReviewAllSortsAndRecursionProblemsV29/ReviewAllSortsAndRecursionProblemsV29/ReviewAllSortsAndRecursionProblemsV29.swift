//
//  ReviewAllSortsAndRecursionProblemsV29.swift
//  ReviewAllSortsAndRecursionProblemsV29
//
//  Created by Don Clore on 4/1/23.
//
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], index: Int, remaining: Int, target: Int) {
    ...
    let excludeRemaining = remaining - (count * candidate)
    helper(build: &build, index: loopindex, remaining: excludeRemaining, target: target)
...
func get_words_from_phone_number(phone_number: String) -> [String] {
  ...
  func helper(build: inout String, index: Int) {
    guard index < phone_number.count else {
      ret.append(build)
      return
    }
