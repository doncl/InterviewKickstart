//
//  ReviewAllSortsAndRecursionProblemsV23.swift
//  ReviewAllSortsAndRecursionProblemsV23
//
//  Created by Don Clore on 3/26/23.
//
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], index: Int, remaining: Int, target: Int) {
    ...
    let excludedRemaining = remaining - (count * candidate)
    helper(build: &build, index: loopindex, remaining: excludedRemaining, target: target)
...
func get_words_from_phone_number(phone_number: String) -> [String] {
  ...
  func helper(build: inout String, index: Int) {
    ...
    let char = phone_number[stringindex]
    
    if let letterArray = map[char] {
...
func find_all_arrangements(n: Int) -> [[String]] {
  ...
  func isConflicted(lastRow: Int, lastColumn: Int, earlierRow: Int, earlierColumn: Int) -> Bool {
  ...
  func helper(build: inout [Int], index: Int) {
    ...
    if index > 1 {
      ...
      for i in 0..<index - 1{
        ...
        if isConflicted(lastRow: lastrow, lastColumn: lastColumn, earlierRow: earlierRow, earlierColumn: earlierColumn) {
          return
        }
      }
    }
