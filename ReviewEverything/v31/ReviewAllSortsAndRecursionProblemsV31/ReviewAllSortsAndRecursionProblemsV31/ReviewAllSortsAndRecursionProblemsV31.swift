//
//  ReviewAllSortsAndRecursionProblemsV31.swift
//  ReviewAllSortsAndRecursionProblemsV31
//
//  Created by Don Clore on 4/5/23.
//
...
func delete_from_bst(root: BinaryTreeNode?, values_to_be_deleted: [Int]) -> BinaryTreeNode? {
  ...
  func search(node: BinaryTreeNode, value: Int) -> (nodeToDelete: BinaryTreeNode, parentNode: BinaryTreeNode?)? {
    var parent: BinaryTreeNode? = nil
    var curr: BinaryTreeNode? = node
    
    while curr != nil {
      if value == curr!.value {
        return (nodeToDelete: curr!, parentNode: parent)
  ...
  func deleteFromBST(root: BinaryTreeNode?, value: Int) -> BinaryTreeNode? {
    ...
    guard let nodeToDeleteTuple = search(node: root, value: value) else {
      return root
    }
    
    let nodeToDelete: BinaryTreeNode = nodeToDeleteTuple.nodeToDelete
    let parent: BinaryTreeNode? = nodeToDeleteTuple.parentNode
  ...
  for int in values_to_be_deleted {
    ret = deleteFromBST(root: ret, value: int)
  }
  return ret
}
