//
//  ReviewAllSortsAndRecursionProblemsV5.swift
//  ReviewAllSortsAndRecursionProblemsV5
//
//  Created by Don Clore on 3/17/23.
//
...
func permuteIntsWithoutRepetitions(arr: [Int]) -> [[Int]] {
  ...
  func helper(build: inout [Int], bank: inout [Int], index: Int) {
    guard build.count < bank.count else {
      ret.append(build)
      return
    }
...
func generate_all_combinations(arr: [Int], target: Int) -> [[Int]] {
  ...
  func helper(build: inout [Int], bank: [Int], index: Int, remaining: Int, target: Int) {
    ...
    let remainingAfterDupsRun = remaining - (dupsCount * candidate)
    
    // exclusion case
    helper(build: &build, bank: bank, index: endOfDupsIndex, remaining: remainingAfterDupsRun, target: target)
    
    // inclusion case
    
    for copies in 1...dupsCount {
      build.append(candidate)
      let localSum = copies * candidate
  ...
  let bank = arr.sorted()
  var build: [Int] = []
  let sumOfInput = arr.reduce(0, +)
  helper(build: &build, bank: bank, index: 0, remaining: sumOfInput, target: target)
